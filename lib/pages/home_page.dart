...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _initializeUser() async {
    ...
    // Como siempre hay un usuario logueado, podemos asegurar que user no es null
  ...
  void _navigateToMenuFinca(Map<String, dynamic> finca) {
    // Aseguramos que el userId esté disponible antes de navegar
    final userId = Supabase.instance.client.auth.currentUser!.id;
    stateManager.setUserId(userId); // Reforzamos que el userId esté establecido

    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => MenuFincaPage(
          fincaId: finca['id'],
          nombreFinca: finca['s_nombrefinca'],
        ),
      ),
    );
  }
                            ...
                            Navigator.push(
                              ...
                              MaterialPageRoute(
                                builder: (context) => MenuFincaPage(
                                  ...
                                  nombreFinca: fincas[index]['s_nombrefinca'],
                                ),
                              ),
                            );
                          },
                      ...
                      onPressed: canCreateFarm
                          ? () async {
                              ...
                              if (user != null) {
                                ...
                                // Reload fincas if a new one was created
                                if (result == true) {
                                  _loadFincas();
                                }
                              }
                            }
                          : null, // Button disabled when can't create more farms
                      style: ElevatedButton.styleFrom(
                        backgroundColor: canCreateFarm
                            ? Color(0xFF34A853)
                            : Colors.grey[350], // Grey when disabled
                        padding:
